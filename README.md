...
## 11. 조회수 기능
- tbl_board 게시글 테이블에 views 컬럼 추가
- update 문으로 단일 게시글 데이터를 불러올때 조회수 컬럼이 +1이 자동으로 update 되도록 설계
...
## 12. 좋아요 / 싫어요 기능
- tbl_board 게시글 테이블에 각 게시글 별로 좋아요와 싫어요 개수를 counting하는 컬럼을 추가
- update 문으로 좋아요 혹은 싫어요 버튼을 눌렀을 때, 컬럼값이 +1이 자동으로 update 되도록 설계
- 좋아요 및 싫어요 단일값의 정보를 저장할 수 있는 tbl_board_like 및 tbl_board_hate 테이블 생성
- 각 좋아요, 싫어요 별로 고유 번호를 지정하고 각각에 글번호, 유저아이디를 매칭
- 좋아요, 싫어요를 취소할경우 각각의 좋아요 및 싫어요에 붙어있는 고유 번호를 기준으로 테이블에서 삭제
- 게시글 테이블에 존재하는 좋아요, 싫어요 컬럼 또한 취소 시 -1 감소
- tbl_member 테이블에 memboer_point 컬럼을 추가하고, 좋아요 혹은 싫어요를 누를때마다 값을 증가시켜 유저별 좋아요, 싫어요 누른 횟수 확인
...
### 13.1. 로그의 중요성
처음 코딩을 공부할 때 에러가 나오면 도대체 어디서부터 어떻게 어디를 고쳐야할지도 몰랐다. 
무작정 오류내용을 구글에 복붙하고 1~10까지 해결방안을 무작정 시도하면서 10가지 중 1가지가 걸리기를 기도할 뿐이었다.
로그라는 기능을 알게된 건 이 프로젝트를 시작하면서 스프링을 공부할 때 알게되었다.

파일 업로드 기능을 구현하는데 ajax 기능이 동작하지 않았다. 알고리즘 풀이 같이 하나의 파일에서 오류 부분을 찾는 것이 아닌, 
프론트부분에서 백단에 걸친 5~6개의 전체 코드 파일중에서 어디에 에러가 발생했는지 찾는 것은 쉽지 않은 일이었다.

스프링 예제에 log를 찍는 코드를 보고 본격적으로 활용해보기 시작했다.

처음에는 무작정 아무데나 로그를 찍어보았지만 점점 로그 찍는 실력이 늘어갔고 오류메시지에 맞춰 로그를 찍기도 하고
논리 오류가 발생한 부분을 예측하면서 로그를 찍기도 했다.

로그를 찍으면서부터 웬만한 오류는 문제없이 해결하는 실력으로 늘어났고, 사실상 나의 개발능력을 기하급수적으로 상승시켜준 정말 좋은 습관이라고 생각한다.
예전처럼 오류가 발생했을 때, 당황하지 않았고 오히려 문제를 해결하는 상황을 즐기는 수준까지 올라갔다.
